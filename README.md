...
- Add `checksum:newlistname`, `generate:newlistname`, `makelist:newlistname` command to `package.json` analogous to Plexswap default and extended list scripts.
- Modify `checksum.ts`, `buildList.ts`, `ci-check.ts`, and `default.test.ts` to handle new list

## How to add new tokens to Plexswap (extended) token list
...
- Add new tokens to `src/tokens/Plexswap-extended.json` file
...
```shell script
# Fetch the Top100 Tokens on Plexswap v2, and update list.
$ yarn fetch:pcs-top-100

# Build token list (Plexswap-top-100.json)
...
Netlify simply takes the json files under `lists` directory and hosts them on `tokens.Plexswap.finance/list-name.json`
